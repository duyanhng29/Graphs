    ...
    public:
        class GNode{
            public:
                ...
                GNode(string l, unordered_map<string, double> n) : label(l), neighbors(n), visisted(false), prev(nullptr), currDist(numeric_limits<double>::max()) {}
                
                bool operator<(const GNode& other){
                    if(currDist != other.currDist){
                        return currDist > other.currDist;
                    }
        ...
        Graph::GNode* BFS_Helper(string const & start, string const & end);
        ...
        private:
            int numNodes;
            int numEdges;
            unordered_map<string, GNode*> map;
};
class minH{
    public:
        bool operator()(Graph::GNode*& n1, Graph::GNode*& n2) const{
                return *n1 < *n2;
